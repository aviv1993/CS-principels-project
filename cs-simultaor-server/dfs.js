class DfsAlgo {
  ...
  dfs(prevRow,prevCol,row,col){
    const key = row +"," + col;
    if(this.isLegalNode(row,col) && !this.blockingNodes.includes(key) && !this.visited.has(key)){
  ...
  run() {
    this.dfs(this.startNode[0],this.startNode[1],this.startNode[0],this.startNode[1]);
